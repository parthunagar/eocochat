...
import 'package:fiberchat/Reasturant/components/file_picker_dialog_component.dart';
  ...
  Widget getImagePlatform({double? height, double? width}) {
    if (!isUpdate) {
      ...
      if (image != null) {
        ...
        return cachedImage(widget.defaultImage, fit: BoxFit.fill, height: height, width: width).cornerRadiusWithClipRRect(defaultRadius);
      }
    }
  }
              ...
              child: IconButton(
                onPressed: () async {
                  FileType file = await showInDialog(
                    ...
                    title: Text(getTranslated(context, 'lblChooseAnAction'), style: boldTextStyle()),
                    builder: (p0) {
