...
import 'package:fiberchat/Screens/newsScreen/WebViewExample.dart';
...
class HomepageState extends State<Homepage> with WidgetsBindingObserver, AutomaticKeepAliveClientMixin, TickerProviderStateMixin {
  ...
  void setLastSeen() async {
    if (currentUserNo != null && widget.currentUserNo != null)
      await FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(currentUserNo).update({Dbkeys.lastSeen: DateTime.now().millisecondsSinceEpoch});
  }
    ...
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) {
      controllerIfcallallowed!.addListener(() {
        if (controllerIfcallallowed!.index == 2) {
          ...
          statusProvider.searchContactStatus(widget.currentUserNo!, contactsProvider.joinedUserPhoneStringAsInServer);
        }
      });
  ...
  detectLocale() async {
    await Devicelocale.currentLocale.then((locale) async {
      ...
      Fiberchat.toast('Error occured while fetching Locale :$onError',);
    });
  }
  ...
  triggerSharing() {
    ...
    if (_sharedText != null) {
      Navigator.push(context, new MaterialPageRoute(builder: (context) => new SelectContactToShare(
        prefs: widget.prefs,
        model: _cachedModel!,
  ...
  unsubscribeToNotification(String? userphone) async {
    ...
    await FirebaseMessaging.instance.unsubscribeFromTopic(Platform.isAndroid
      ? Dbkeys.topicUSERSandroid
      : Platform.isIOS ? Dbkeys.topicUSERSios : Dbkeys.topicUSERSweb).catchError((err) {
      print(err.toString());
    });
  }
  ...
  logout(BuildContext context) async {
    ...
    Navigator.of(context).pushAndRemoveUntil(
      // the new route
      MaterialPageRoute(builder: (BuildContext context) => FiberchatWrapper()),
  ...
  void listenToNotification() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      debugPrint("message.data['title'] 1 : ${message.data['title']}");
      if (message.data['title'] != 'Call Ended' &&
        ...
        debugPrint("message.data['title'] 2 : ${message.data['title']}");
        // await FirebaseMessaging.instance.setForegroundNotificationPresentationOptions(alert: true, badge: true, sound: true);
        ...
        if (message.data['title'] == 'Call Ended') {
          ...
          if (message.data['title'] == 'Incoming Audio Call...' || message.data['title'] == 'Incoming Video Call...') {
            ...
            if (currentpeer.peerid != message.data['peerid']) {
              ...
              showOverlayNotification((context) {
                return Card(
                  ...
                  child: SafeArea(
                    child: ListTile(
                      title: Text(message.data['titleMultilang'], maxLines: 1, overflow: TextOverflow.ellipsis),
                      subtitle: Text(message.data['bodyMultilang'], maxLines: 2, overflow: TextOverflow.ellipsis),
                      trailing: IconButton(
                        icon: Icon(Icons.close),
                        onPressed: () {
                          OverlaySupportEntry.of(context)!.dismiss();
            ...
            debugPrint("message.data['title'] : ${message.data['title']}");
            showOverlayNotification((context) {
              return Card(
                margin: const EdgeInsets.symmetric(horizontal: 4),
                child: SafeArea(
                  child: ListTile(
                    leading: Image.network(message.data['image'], width: 50, height: 70, fit: BoxFit.cover),
                    title: Text(message.data['titleMultilang'], maxLines: 1, overflow: TextOverflow.ellipsis),
                    subtitle: Text(message.data['bodyMultilang'], maxLines: 2, overflow: TextOverflow.ellipsis),
                    trailing: IconButton(
                      icon: Icon(Icons.close),
...
//  These firestore documents will be automatically in the first run set if Admin app is required but not configured yet. You need to edit all the default settings through admin app-----
        ...
        if (doc.exists && doc.data()!.containsKey(Dbkeys.usersidesetupdone)) {
          if (!doc.data()!.containsKey(Dbkeys.updateV7done)) {
            ...
            setState(() {
              ...
              print('isblockNewlogins : $isblockNewlogins');
              isApprovalNeededbyAdminForNewUser = doc[Dbkeys.isaccountapprovalbyadminneeded];
              print('isApprovalNeededbyAdminForNewUser : $isApprovalNeededbyAdminForNewUser');
              accountApprovalMessage = doc[Dbkeys.accountapprovalmessage];
              print('accountApprovalMessage : $accountApprovalMessage');
            });
            if (doc[Dbkeys.isemulatorallowed] == false && mapDeviceInfo[Dbkeys.deviceInfoISPHYSICAL] == false) {
              ...
              if (doc[Platform.isAndroid
                  ...
                  : Platform.isIOS ? Dbkeys.isappunderconstructionios : Dbkeys.isappunderconstructionweb] == true) {
                ...
                double currentNewAppVersionInServer =
                  double.parse(doc[Platform.isAndroid
                    ...
                    : Platform.isIOS ? Dbkeys.latestappversionios : Dbkeys.latestappversionweb].trim().replaceAll(".", ""));
                ...
                if (currentAppVersionInPhone < currentNewAppVersionInServer) {
                  showDialog<String>(
                    ...
                    builder: (BuildContext context) {
                      ...
                      return new WillPopScope(
                        ...
                        child: AlertDialog(
                          title: Text(title, style: TextStyle(color: fiberchatDeepGreen)),
                          content: Text(message),
                          actions: [
                            TextButton(
                              child: Text(btnLabel, style: TextStyle(color: fiberchatLightGreen)),
                              onPressed: () => launch(doc[Platform.isAndroid
                                ? Dbkeys.newapplinkandroid
                                : Platform.isIOS ? Dbkeys.newapplinkios : Dbkeys.newapplinkweb])),
                          ],
                        ));
                    },
                  );
                  ...
                  if (currentUserNo == null || currentUserNo!.isEmpty || widget.isSecuritySetupDone == false) {
                    await unsubscribeToNotification(widget.currentUserNo);
                    print(' =============> LoginScreen 1 <============= ');
                    unawaited(Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) =>
                      new LoginScreen(
                        prefs: widget.prefs,
          ...
          await setupAdminAppCompatibleDataForFirstTime().then((result) {
            if (result == true) {
              ...
              Fiberchat.toast('Error occured while writing setupAdminAppCompatibleDataForFirstTime().Please restart the app.',);
            }
          });
        }
      }).catchError((err) {
        Fiberchat.toast('Error occured while fetching appsettings/userapp. ERROR: $err',);
      });
    } else {
      await FirebaseFirestore.instance.collection('version').doc('userapp').get().then((doc) async {
        if (doc.exists) {
          ...
          if (currentAppVersionInPhone < currentNewAppVersionInServer) {
            ...
            // ignore: unnecessary_null_comparison
            if (currentUserNo == null || currentUserNo!.isEmpty || widget.isSecuritySetupDone == false || widget.isSecuritySetupDone == null)

             {
               print(' =============> LoginScreen 2 <============= ');
               unawaited(Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) => new LoginScreen(
                 prefs: widget.prefs,
                 accountApprovalMessage: accountApprovalMessage,
                 isaccountapprovalbyadminneeded: isApprovalNeededbyAdminForNewUser,
                 isblocknewlogins: isblockNewlogins,
          ...
          await FirebaseFirestore.instance.collection('version').doc('userapp').set({'version': '1.0.0', 'url': 'https://www.google.com/'},
              SetOptions(merge: true));
          Fiberchat.toast(getTranslated(context, 'setup'));
        }
      }).catchError((err) {
        print('FETCHING ERROR AT INITIAL STARTUP: $err');
        Fiberchat.toast(getTranslated(context, 'loadingfailed') + err.toString());
      });
    }
  }
  ...
  void _changeLanguage(Language setLanguage) async {
    // Locale _locale = await setLocale(language.languageCode);
    print('_changeLanguage => setLanguage-id : ${setLanguage.id}');
    print('_changeLanguage => setLanguage-flag : ${setLanguage.flag}');
    print('_changeLanguage => setLanguage-name : ${setLanguage.name}');
    print('_changeLanguage => setLanguage-languageCode : ${setLanguage.languageCode}');
    print('_changeLanguage => setLanguage-languageNameInEnglish : ${setLanguage.languageNameInEnglish}');
    Locale _locale = await setLocale(setLanguage.languageCode);
    print('_changeLanguage => _locale : $_locale');
    FiberchatWrapper.setLocale(context, _locale);
    setState(() {
    ...
    if (currentUserNo != null) {
      print('_changeLanguage => currentUserNo : $currentUserNo');
  ...
  Future<bool> onWillPop() {
    ...
    if (now.difference(currentBackPressTime!) > Duration(seconds: 3)) {
      ...
      Fiberchat.toast(getTranslated(context, 'doubleTapToGoBack'));
                    ...
                    : PickupLayout(
                        ...
                        child: Scaffold(
                            ...
                            appBar: AppBar(
                                ...
                                title: Text(
                                  Appname,
                                  style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack, fontSize: 20.0, fontWeight: FontWeight.w600)),
                                actions: <Widget>[
                                  Container(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            Icon(
                                              ...
                                              color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack.withOpacity(0.7),
                                              size: 22),
                                            SizedBox(width: 4),
                                            Icon(
                                              Icons.keyboard_arrow_down,
                                              color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatLightGreen : fiberchatBlack.withOpacity(0.7),
                                              size: 27)
                                          ],
                                        ),
                                      ),
                                      onChanged: (Language? language) {
                                        print('onChanged => language : $language');
                                      ...
                                      items: Language.languageList().map<DropdownMenuItem<Language>>(
                                        (e) => DropdownMenuItem<Language>(
                                          ...
                                          child: Row(
                                            ...
                                            children: <Widget>[
                                              Text(' ' + e.languageNameInEnglish + '  ' + e.flag + ' ', style: TextStyle(fontSize: 15)),
                                            ],
                                          ),
                                        ),
                                      ).toList(),
                                  ...
                                  Container(
                                      ...
                                      child: IconButton(
                                        onPressed: () {
                                          ...
                                          Navigator.push(context, new MaterialPageRoute(builder: (context) => AllNotifications()));
                                          // //ADD THIS
                                          // Navigator.push(context, new MaterialPageRoute(builder: (context) => AddToWalletScreen()));
...
// // //---- All localizations settings----
                                  PopupMenuButton(
                                        ...
                                        switch (val) {
                                            ...
                                            showDialog(
                                                ...
                                                builder: (context) {
                                                  return SimpleDialog(
                                                    ...
                                                    children: <Widget>[
                                                      ListTile(title: Text(getTranslated(context, 'swipeview'))),
                                                      SizedBox(height: 10),
                                                      ListTile(title: Text(getTranslated(context, 'swipehide'))),
                                                      SizedBox(height: 10),
                                                      ListTile(title: Text(getTranslated(context, 'lp_setalias')))
                                                    ],
                                                  );
                                                });
                                            break;
                                          ...
                                          case 'settings':
                                            Navigator.push(context, new MaterialPageRoute(builder: (context) =>
                                                SettingsOption(
                                                  onTapLogout: () async { await logout(context);  },
                                                  onTapEditProfile: () {
                                                    ...
                                                    prefs: widget.prefs,
                                                    onSuccess: () {
                                                      Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) => ProfileSetting(
                                                        prefs: widget.prefs,
                                          ...
                                          case 'group':
                                            if (observer.isAllowCreatingGroups == false) {
                                              ...
                                              final AvailableContactsProvider dbcontactsProvider = Provider.of<AvailableContactsProvider>(context, listen: false);
                                              dbcontactsProvider.fetchContacts(context, _cachedModel, widget.currentUserNo!, widget.prefs);
                                          ...
                                          case 'broadcast':
                                            if (observer.isAllowCreatingBroadcasts == false) {
                                              ...
                                              final AvailableContactsProvider dbcontactsProvider = Provider.of<AvailableContactsProvider>(context, listen: false);
                                              dbcontactsProvider.fetchContacts(context, _cachedModel, widget.currentUserNo!, widget.prefs);
                                              await Navigator.push(context, MaterialPageRoute(builder: (context) => AddContactsToBroadcast(
                                                currentUserNo: widget.currentUserNo,
                                                model: _cachedModel,
                                                biometricEnabled: false,
                                                prefs: widget.prefs,
                                      ...
                                      itemBuilder: (context) =>
                                          <PopupMenuItem<String>>[
                                            PopupMenuItem<String>(value: 'group', child: Text(getTranslated(context, 'newgroup'))),
                                            // //TODO:---- REMOVE BROADCAST FEATURE IF NOT NEEDED--------
                                            PopupMenuItem<String>(value: 'broadcast', child: Text(getTranslated(context, 'newbroadcast'))),
                                            PopupMenuItem<String>(value: 'tutorials', child: Text(getTranslated(context, 'tutorials'),)),
                                            PopupMenuItem<String>(value: 'settings', child: Text(getTranslated(context, 'settingsoption'))),
                                          ]),
                                ],
                                bottom: TabBar(
                                  labelStyle: TextStyle(fontWeight: FontWeight.bold),
                                  unselectedLabelStyle: TextStyle(fontWeight: FontWeight.bold),
                                  labelColor: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack,
                                  unselectedLabelColor: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite.withOpacity(0.6) : fiberchatBlack.withOpacity(0.6),
                                  indicatorWeight: 3,
                                  indicatorColor: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatgreen,
                                  ...
                                  tabs: observer.isCallFeatureTotallyHide == false
                                      ? <Widget>[
                                          // Tab(icon: Icon(Icons.search, size: 22)),
                                          Tab(child: Text(getTranslated(context, 'news'), textAlign: TextAlign.center,)),
                                          Tab(child: Text(getTranslated(context, 'chats'), textAlign: TextAlign.center,)),
                                          Tab(child: Text(getTranslated(context, 'status'), textAlign: TextAlign.center)),
                                          Tab(child: Text(getTranslated(context, 'calls'), textAlign: TextAlign.center)),
                                        ]
                                      : <Widget>[
                                          // Tab(icon: Icon(Icons.search, size: 22)),
                                    Tab(child: Text(getTranslated(context, 'news'), textAlign: TextAlign.center,)),
                            ...
                            body: TabBarView(
                              controller:
                                  observer.isCallFeatureTotallyHide == false ? controllerIfcallallowed : controllerIfcallNotallowed,
    ...
    body: Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.error_outline_outlined, size: 60, color: Colors.yellowAccent),
            SizedBox(height: 30),
            Text('$title', textAlign: TextAlign.center, style: TextStyle(fontSize: 20, color: fiberchatWhite, fontWeight: FontWeight.w700)),
            SizedBox(height: 20),
            Text('$subtitle', textAlign: TextAlign.center, style: TextStyle(fontSize: 17, color: fiberchatWhite.withOpacity(0.7), fontWeight: FontWeight.w400),)
          ],
        ),
      ),
    ),
