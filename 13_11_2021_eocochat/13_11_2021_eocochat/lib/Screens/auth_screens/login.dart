...
import 'package:fiberchat/Screens/auth_screens/register_screen.dart';
...
class LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> verifyPhoneNumber() async {
    ...
    final PhoneVerificationFailed verificationFailed = (FirebaseAuthException authException) {
      ...
      print('Authentication failed -ERROR: ${authException.message}. Try again later.');
  ...
  Future<Null> handleSignIn({AuthCredential? authCredential}) async {
    ...
    print('handleSignIn => phoneNo : $phoneNo');

    AuthCredential credential;
    if (authCredential == null)
      credential = PhoneAuthProvider.credential(verificationId: verificationId!, smsCode: _code);
    else
      credential = authCredential;
      UserCredential firebaseUser;

    firebaseUser = await firebaseAuth.signInWithCredential(credential).catchError((err) async {
      if (err.toString().contains('return a value') || err.toString().contains('null')) {
      } else {
    ...
    if (firebaseUser != null) {
      ...
      if (documents.isEmpty) {
        ...
        await FirebaseFirestore.instance.collection(DbPaths.collectionnotifications).doc(DbPaths.adminnotifications).update({
          ...
          Dbkeys.nOTIFICATIONxxdesc: widget.isaccountapprovalbyadminneeded == true
        ...
        //         unawaited(Navigator.pushReplacement(newContext,
        ...
        await FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(phoneNo).update(!documents[0].data().containsKey(Dbkeys.deviceDetails)
                  ? {
                      Dbkeys.authenticationType: AuthenticationType.passcode.index,
                      Dbkeys.accountstatus: widget.isaccountapprovalbyadminneeded == true ? Dbkeys.sTATUSpending : Dbkeys.sTATUSallowed,
                      Dbkeys.actionmessage: widget.accountApprovalMessage,
                      ...
                      Dbkeys.deviceDetails: mapDeviceInfo,
                      Dbkeys.currentDeviceID: deviceid,
                      Dbkeys.phonenumbervariants: phoneNumberVariantsList(countrycode: documents[0][Dbkeys.countryCode], phonenumber: documents[0][Dbkeys.phoneRaw])
                    }
                  : {
                      Dbkeys.searchKey: _name.text.trim().substring(0, 1).toUpperCase(),
                      Dbkeys.nickname: _name.text.trim(),
        ...
        // ignore: unnecessary_null_comparison
        // if (widget.issecutitysetupdone == false || widget.issecutitysetupdone == null) {
        //   unawaited(Navigator.pushReplacement(this.context, MaterialPageRoute(builder: (context) => Security(
        //     phoneNo,
        //     prefs: widget.prefs,
        //     setPasscode: true,
        //     onSuccess: (newContext) async {
        //       unawaited(Navigator.pushReplacement(newContext, MaterialPageRoute(builder: (newContext) => Homepage(currentUserNo: phoneNo, isSecuritySetupDone: true, prefs: widget.prefs,))));
        //     },
        //     title: getTranslated(this.context, 'authh'),
        //   ))));
        // } else {
  ...
  callLoginApi(String phoneNo) {
    print('callLoginApi => phoneNo : $phoneNo');
    APIServices.getLoginData('${phoneNo.replaceAll('+', '')}').then((loginValue) async {
      print('loginValue 1 : $loginValue');
      if(loginValue == null) {
        Navigator.push(context, MaterialPageRoute(builder: (context) => RegisterScreen(prefs: widget.prefs)));
      }
      else {
        await widget.prefs.setString(Dbkeys.sharedPrefEmail,loginValue.email!);
        await widget.prefs.setString(Dbkeys.sharedPrefMobile,loginValue.mobile!);
        await widget.prefs.setString(Dbkeys.sharedPrefUsername,loginValue.username!);
        await widget.prefs.setString(Dbkeys.sharedPrefCountryCode,loginValue.countryCode!);
        await widget.prefs.setString(Dbkeys.sharedPrefAccountNumber,loginValue.accountNumber!);
        await widget.prefs.setString(Dbkeys.sharedPrefUpdatedAt,loginValue.updatedAt!.toString());
        await widget.prefs.setString(Dbkeys.sharedPrefCreatedAt,loginValue.createdAt!.toString());
        await widget.prefs.setString(Dbkeys.sharedPrefUserId,loginValue.id!.toString());

        await widget.prefs.setString(Dbkeys.sharedPrefFcmToken,loginValue.deviceToken.toString());
        ...
        //     setPasscode: true,
        //     onSuccess: () async {
        //       unawaited(Navigator.push(context, new MaterialPageRoute(builder: (context) => EocochatWrapper())));
        //       Eocochat.toast(getTranslated(this.context, 'welcometo') + ' $Appname');
        //     },
        //     title: getTranslated(this.context, 'authh'),
        //   ))));
        if (widget.issecutitysetupdone == false || widget.issecutitysetupdone == null) {
          unawaited(Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Security(
            phoneNo,
            prefs: widget.prefs,
            setPasscode: true,
            onSuccess: () async {
              unawaited(Navigator.pushReplacement(context,new MaterialPageRoute(builder: (context) => FiberchatWrapper())));
            },
            title: getTranslated(this.context, 'authh'),
          ))));
        } else {
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: DESIGN_TYPE == Themetype.whatsapp? [fiberchatgreen, fiberchatDeepGreen]: [fiberchatWhite, fiberchatWhite]),
        ),
        child: Column(
          children: <Widget>[
                  ...
                  Container(
                    ...
                    child: DropdownButton<Language>(
                      ...
                      icon: Row(
                        ...
                        children: [
                          Icon(Icons.language_outlined, color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack.withOpacity(0.8)),
                          SizedBox(width: 2),
                          SizedBox(width: 15, child: Icon(Icons.keyboard_arrow_down, color: fiberchatLightGreen, size: 27))
                        ],
                      ),
                      ...
                      items: Language.languageList().map<DropdownMenuItem<Language>>((e) => DropdownMenuItem<Language>(
                        ...
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.end,
                          children: <Widget>[Text(' ' + e.languageNameInEnglish + '  ' + e.flag + ' ')],
                        ),
                      ),
                    ).toList(),
                    ),
                        ...
                        Container(
                          decoration: BoxDecoration(
                            boxShadow: [
                              BoxShadow(
                                blurRadius: 3.0,
                                color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatDeepGreen.withOpacity(0.3) : fiberchatBlack.withOpacity(0.1),
                                spreadRadius: 1.0),
                            ],
                          ...
                          child: Column(
                            children: <Widget>[
                              ...
                              Container(
                                ...
                                width: w / 1.24,
                                child: InpuTextBox(
                                  inputFormatter: [LengthLimitingTextInputFormatter(25)],
                                  controller: _name,
                                  leftrightmargin: 0,
                                  ...
                                  child: MobileInputWithOutline(
                                    ...
                                    onSaved: (phone) {
                                      setState(() {
                                        ...
                                        print('onSaved => phone.completeNumber : ${phone.completeNumber}');
                                        print('onSaved => phone.countryISOCode : ${phone.countryISOCode}');
                                        print('onSaved => phone.countryCode : ${phone.countryCode}');
                                        print('onSaved => phone.number : ${phone.number}');
                                        print('onSaved => phoneCode : $phoneCode');
                              ...
                              Padding(
                                ...
                                child: isLoading == true
                                    ...
                                    : MySimpleButton(
                                        spacing: 0.3,
                                        height: 57,
                                        buttoncolor: DESIGN_TYPE == Themetype.whatsapp ? fiberchatLightGreen : fiberchatLightGreen,
                                        buttontext: getTranslated(this.context, 'sendverf'),
                                        onpressed: widget.isblocknewlogins == true
                                            ? () {  Fiberchat.toast(getTranslated(this.context, 'logindisabled'));  }
                                            : () {
                                                setState(() {});
                                    ...
                                    text: TextSpan(
                                      children: <TextSpan>[
                                        TextSpan(
                                            ...
                                            style: TextStyle(
                                                color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite.withOpacity(0.8) : fiberchatBlack.withOpacity(0.8),
                                                fontWeight: FontWeight.w400, fontSize: 14.0, height: 1.7)),
                                        TextSpan(
                                          text: getTranslated(this.context, 'tnc'),
                                          style: TextStyle(height: 1.7, color: fiberchatLightGreen, fontWeight: FontWeight.w700, fontSize: 14.8)),
                                        TextSpan(
                                            text: '  ○  ',
                                            style: TextStyle(height: 1.7, color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite.withOpacity(0.79) : fiberchatBlack.withOpacity(0.79), fontWeight: FontWeight.w500, fontSize: 11.8)),
                                        TextSpan(
                                ...
                                child: RichText(
                                  ...
                                  text: TextSpan(
                                    children: <TextSpan>[
                                      TextSpan(
                                          text: '${getTranslated(this.context, 'agree')} \n',
                                          style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite.withOpacity(0.8) : fiberchatBlack.withOpacity(0.8), fontWeight: FontWeight.w400, fontSize: 14.0, height: 1.7)),
                                      TextSpan(
                                          text: getTranslated(this.context, 'tnc'),
                                          style: TextStyle(height: 1.7, color: fiberchatLightGreen, fontWeight: FontWeight.w700, fontSize: 14.8),
                                              ...
                                              if (ConnectWithAdminApp == false) {
                                                ...
                                                if (observer.tncType == 'url') {
                                                  ...
                                                  Navigator.push(this.context, MaterialPageRoute(builder: (context) =>
                                                      PDFViewerCachedFromUrl(title: getTranslated(this.context, 'tnc'), url: observer.tnc)));
                                                }
                                              }
                                            }),
                                      TextSpan(
                                          ...
                                          style: TextStyle(
                                              height: 1.7,
                                              color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite.withOpacity(0.79) : fiberchatBlack.withOpacity(0.79),
                                            ...
                                            ..onTap = () {
                                              if (ConnectWithAdminApp == false) {
                                                ...
                                                if (observer.privacypolicyType == 'url') {
                                                  ...
                                                  Navigator.push(this.context, MaterialPageRoute(builder: (context) =>
                                                      PDFViewerCachedFromUrl(title: getTranslated(this.context, 'pp'), url: observer.privacypolicy)));
                                                }
                                              }
                                            }),
                                    ],
                  ...
                  : Container(
                      decoration: BoxDecoration(
                        boxShadow: [
                          BoxShadow(
                            blurRadius: 3.0,
                            color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatgreen.withOpacity(0.3) : fiberchatBlack.withOpacity(0.1),
                            spreadRadius: 1.0,
                          ),
                        ],
                        borderRadius: BorderRadius.circular(20),
                        ...
                        children: <Widget>[
                            ...
                            child: Padding(
                              ...
                              child: PinFieldAutoFill(
                                ...
                                decoration: UnderlineDecoration(
                                  bgColorBuilder: FixedColorBuilder(fiberchatGrey.withOpacity(0.1)),
                                  textStyle: TextStyle(fontSize: 22, color: fiberchatBlack, fontWeight: FontWeight.bold),
                                  colorBuilder: FixedColorBuilder(fiberchatGrey.withOpacity(0.1)),
                                ),
                                currentCode: _code,
                          ...
                          InkWell(
                            ...
                            child: Padding(
                                ...
                                child: Center(
                                  child: Text(getTranslated(this.context, 'back'), style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                                )),
                          ),
                          //
                          SizedBox(height: 18),
...
class MobileInputWithOutline extends StatefulWidget {
  ...
  final String? buttonText;
  final Function(PhoneNumber? phone)? onSaved,onChanged,onCountryChanged;
  ...
  MobileInputWithOutline(
      ...
      this.buttonText,this.onChanged,this.onCountryChanged});
  @override
