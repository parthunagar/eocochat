...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) {
      print('chat initState => timeStamp : $timeStamp');
  ...
  dynamic encryptWithCRC(String input) {
    try {
      ...
      Fiberchat.toast(getTranslated(this.context, 'waitingpeer'));
      return false;
    }
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed)
     {
       print('state => AppLifecycleState.resumed : ${AppLifecycleState.resumed}');
       setIsActive();
     }
    else
      {
  ...
  readLocal(BuildContext context) async {
    ...
    textEditingController.addListener(() {
      if (textEditingController.text.isNotEmpty && typing == false) {
        lastSeen = peerNo;
        FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(currentUserNo).update({Dbkeys.lastSeen: peerNo});
        typing = true;
      }
      if (textEditingController.text.isEmpty && typing == true) {
        lastSeen = true;
        FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(currentUserNo).update({Dbkeys.lastSeen: true});
        typing = false;
      }
    });
    setIsActive();
  ...
  String getLastSeenKey() {
    print('getLastSeenKey => lastSeen : $peerNo-${Dbkeys.lastSeen}');
  ...
  getFileData(File image, {int? timestamp, int? totalFiles}) {
    ...
    return observer.isPercentProgressShowWhileUploading
        ? (totalFiles == null ? uploadFileWithProgressIndicator(false, timestamp: timestamp)
        : totalFiles == 1 ? uploadFileWithProgressIndicator(false, timestamp: timestamp)
  ...
  Future uploadFileWithProgressIndicator(bool isthumbnail, {int? timestamp}) async {
    uploadTimestamp = timestamp ?? DateTime.now().millisecondsSinceEpoch;
                    ...
                    builder: (BuildContext context, snapshot) {
                      if (snapshot.hasData) {
                        ...
                        return openUploadDialog(
                          ...
                          title: isthumbnail == true ? getTranslated(context, 'generatingthumbnail') : getTranslated(context, 'sending'),
  ...
  void onSendMessage(BuildContext context, String content, MessageType type, int? timestamp, {bool isForward = false}) async {
    final observer = Provider.of<Observer>(this.context, listen: false);
    if (content.trim() != '') {
      try {
        ...
        if (encrypted is String) {
          ...
          setStateIfMounted(() {
            ...
            messages = List.from(messages)
              ..add(Message(
                ...
                onTap: (tempDoc[Dbkeys.from] == widget.currentUserNo && tempDoc[Dbkeys.hasSenderDeleted] == true) == true
                    ? () {}
          ...
          if (type == MessageType.doc || type == MessageType.audio ||
              // (type == MessageType.image && !content.contains('giphy')) ||
              type == MessageType.location || type == MessageType.contact && widget.isSharingIntentForwarded == false) {
  ...
  updateDeleteBySenderField(int? ts, updateDoc, context) {
    setStateIfMounted(() {
      ...
      if (i >= 0) {
        messages.removeWhere((msg) => msg.timestamp == ts);
        messages.insert(i, Message(child,
            timestamp: timestamp,
            from: from,
            onTap: () {},
  ...
  contextMenuForSavedMessage(BuildContext context, Map<String, dynamic> doc) {
    List<Widget> tiles = List.from(<Widget>[]);
    tiles.add(ListTile(
      dense: true,
      leading: Icon(Icons.delete_outline),
      title: Text(getTranslated(this.context, 'delete'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      onTap: () async {
  ...
  contextMenuNew(contextForDialog, Map<String, dynamic> mssgDoc, bool isTemp, {bool saved = false}) {
    ...
    if ((mssgDoc[Dbkeys.from] == currentUserNo && mssgDoc[Dbkeys.hasSenderDeleted] == false) && saved == false) {
      tiles.add(ListTile(
          dense: true,
          leading: Icon(Icons.delete_outline),
          title: Text(getTranslated(contextForDialog, 'dltforme'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () async {
            Fiberchat.toast(getTranslated(contextForDialog, 'deleting'));
            await FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).collection(chatId!).doc('${mssgDoc[Dbkeys.timestamp]}').get().then((chatDoc) async {
              if (!chatDoc.exists) {
                ...
                if (realtimeDoc[Dbkeys.hasRecipientDeleted] == true) {
                  if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true ? mssgDoc[Dbkeys.isbroadcast] : false) == true) {
                    ...
                    Future.delayed(const Duration(milliseconds: 300), () {
                      ...
                      Fiberchat.toast(getTranslated(contextForDialog, 'deleted'));
                  ...
                  FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).collection(chatId!).doc('${realtimeDoc[Dbkeys.timestamp]}')
      ...
      tiles.add(ListTile(
          ...
          title: Text(getTranslated(contextForDialog, 'dltforeveryone'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () async {
            if ((mssgDoc.containsKey(Dbkeys.isbroadcast) == true ? mssgDoc[Dbkeys.isbroadcast] : false) == true) {
    ...
    if (mssgDoc.containsKey(Dbkeys.broadcastID) && mssgDoc[Dbkeys.to] == widget.currentUserNo) {
      tiles.add(ListTile(
        ...
        onTap: () {
          Fiberchat.toast(getTranslated(contextForDialog, 'plswait'));
          Future.delayed(const Duration(milliseconds: 200), () {
            FirebaseFirestore.instance.collection(DbPaths.collectionbroadcasts).doc(mssgDoc[Dbkeys.broadcastID]).update({
              ...
              hidekeyboard(contextForDialog);
              Fiberchat.toast(getTranslated(contextForDialog, 'blockedbroadcast'));
            }).catchError((error) {
              Navigator.pop(contextForDialog);
              hidekeyboard(contextForDialog);
            });
    ...
    if (mssgDoc[Dbkeys.messageType] == MessageType.text.index && !mssgDoc.containsKey(Dbkeys.broadcastID)) {
      tiles.add(ListTile(
          ...
          title: Text(getTranslated(contextForDialog, 'copy'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () {
            Clipboard.setData(ClipboardData(text: mssgDoc[Dbkeys.content]));
            Navigator.pop(contextForDialog);
            hidekeyboard(contextForDialog);
            Fiberchat.toast(getTranslated(contextForDialog, 'copied'));
          }));
    }
    if (((mssgDoc[Dbkeys.from] == currentUserNo && mssgDoc[Dbkeys.hasSenderDeleted] == false) ||
        (mssgDoc[Dbkeys.to] == currentUserNo && mssgDoc[Dbkeys.hasRecipientDeleted] == false)) == true) {
      tiles.add(ListTile(
          dense: true,
          leading: Icon(FontAwesomeIcons.share, size: 22),
          title: Text(getTranslated(contextForDialog, 'forward'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () async {
            Navigator.of(contextForDialog).pop();
            Navigator.push(contextForDialog, MaterialPageRoute(builder: (contextForDialog) =>
  ...
  sendForwardMessageEach(int index, List<DocumentSnapshot<dynamic>> list, String privateKey, var mssgDoc) async {
    if (index > list.length) {
      ...
      if (list[index].data()!.containsKey(Dbkeys.groupNAME)) {
        try {
          ...
          FirebaseFirestore.instance.collection(DbPaths.collectiongroups).doc(groupDoc[Dbkeys.groupID]).collection(DbPaths.collectiongroupChats).doc(timestamp.toString() + '--' + widget.currentUserNo!).set({
        ...
        try {
          String? sharedSecret = (await e2ee.X25519().calculateSharedSecret(
            ...
            e2ee.Key.fromBase64(list[tempSendIndex][Dbkeys.publicKey], true))).toBase64();
          final key = encrypt.Key.fromBase64(sharedSecret);
          cryptor = new encrypt.Encrypter(encrypt.Salsa20(key));
          ...
          if (encrypted is String) {
            ...
            if (content.trim() != '') {
              Map<String, dynamic>? targetPeer = widget.model.userData[list[tempSendIndex][Dbkeys.phone]];
              if (targetPeer == null) {
                await ChatController.request(currentUserNo, list[tempSendIndex][Dbkeys.phone], Fiberchat.getChatId(widget.currentUserNo, list[tempSendIndex][Dbkeys.phone]));
              }
              ...
              await FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).set({
                ...
                Future messaging = FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(list[tempSendIndex][Dbkeys.phone]).collection(Dbkeys.chatsWith).doc(Dbkeys.chatsWith).set({
                  widget.currentUserNo!: 4,
                }, SetOptions(merge: true));
                await widget.model.addMessage(list[tempSendIndex][Dbkeys.phone], timestamp2, messaging);
              }).then((value) async {
                Future messaging = FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).collection(chatId).doc('$timestamp2').set({
  ...
  contextMenuOld(BuildContext context, Map<String, dynamic> doc, {bool saved = false}) {
    ...
    if (doc[Dbkeys.messageType] == MessageType.text.index) {
      tiles.add(ListTile(
          dense: true,
          leading: Icon(Icons.content_copy),
          title: Text(getTranslated(context, 'copy'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () {
            Clipboard.setData(ClipboardData(text: doc[Dbkeys.content]));
            Navigator.pop(context);
            Fiberchat.toast(getTranslated(this.context, 'copied'));
          }));
    }
    if (doc.containsKey(Dbkeys.broadcastID) && doc[Dbkeys.to] == widget.currentUserNo) {
      tiles.add(ListTile(
          dense: true,
          leading: Icon(Icons.block),
          title: Text(getTranslated(this.context, 'blockbroadcast'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          onTap: () {
            Fiberchat.toast(getTranslated(this.context, 'plswait'));
            Future.delayed(const Duration(milliseconds: 500), () {
              FirebaseFirestore.instance.collection(DbPaths.collectionbroadcasts).doc(doc[Dbkeys.broadcastID]).update({
                Dbkeys.broadcastMEMBERSLIST: FieldValue.arrayRemove([widget.currentUserNo]),
                Dbkeys.broadcastBLACKLISTED: FieldValue.arrayUnion([widget.currentUserNo]),
              }).then((value) {
                Fiberchat.toast(getTranslated(this.context, 'blockedbroadcast'));
                hidekeyboard(context);
                Navigator.pop(context);
              }).catchError((error) {
                Fiberchat.toast(getTranslated(this.context, 'blockedbroadcast'));
                ...
                ? doc[Dbkeys.isForward] == true
                    ? Column(
                        ...
                        children: [
                          Container(
                            child: Row(
                              mainAxisAlignment: isMe == true ? MainAxisAlignment.start : MainAxisAlignment.end,
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(FontAwesomeIcons.share, size: 12, color: fiberchatGrey.withOpacity(0.5)),
                                SizedBox(width: 5),
                                Text(getTranslated(this.context, 'forwarded'),
                                  maxLines: 1,
                                  style: TextStyle(color: fiberchatGrey.withOpacity(0.7), fontStyle: FontStyle.italic, overflow: TextOverflow.ellipsis, fontSize: 13))
                            ])),
                          SizedBox(height: 10),
                          selectablelinkify(doc[Dbkeys.content], 16),
                        ],
  ...
  Widget getTempTextMessage(String message, Map<String, dynamic> doc) {
          ...
          ? doc[Dbkeys.isForward] == true
              ? Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 10),
                    Image.asset('assets/images/mapview.jpg')
                  ])
              : Image.asset('assets/images/mapview.jpg')
          : Image.asset('assets/images/mapview.jpg'),
    );
  ...
  Widget getDocmessage(BuildContext context, Map<String, dynamic> doc, String message, {bool saved = false}) {
    final bool isMe = doc[Dbkeys.from] == currentUserNo;
    return SizedBox(
      width: 220,
      height: 116,
        ...
        children: [
          ...
          ListTile(
            ...
            leading: Container(
              ...
              child: Icon(Icons.insert_drive_file, size: 25, color: Colors.white)),
            title: Text(
          ...
          message.split('-BREAK-')[1].endsWith('.pdf')
              ? Row(
                  ...
                  children: [
                    ...
                    FlatButton(
                        onPressed: () {
                          Navigator.push(context, MaterialPageRoute<dynamic>(builder: (_) =>
      ...
      child: Column(
        ...
        children: [
          ...
          saved
            ? Material(
                child: Container(
                  decoration: BoxDecoration(image: DecorationImage(image: Save.getImageFromBase64(doc[Dbkeys.content]).image, fit: BoxFit.cover)),
            ...
            : CachedNetworkImage(
                placeholder: (context, url) => Container(
                  child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Colors.blueGrey[400]!)),
                  width: doc[Dbkeys.content].contains('giphy') ? 120 : 200.0,
  ...
  Widget getVideoMessage(BuildContext context, Map<String, dynamic> doc, String message, {bool saved = false}) {
    Map<dynamic, dynamic>? meta = jsonDecode((message.split('-BREAK-')[2]).toString());
    final bool isMe = doc[Dbkeys.from] == currentUserNo;
    return InkWell(
      onTap: () {
        Navigator.push(this.context, new MaterialPageRoute(builder: (context) =>
           new PreviewVideo(
              ...
              children: [
                CachedNetworkImage(
                  placeholder: (context, url) => Container(
                    ...
                    height: 197,
                    padding: EdgeInsets.all(80.0),
                    decoration: BoxDecoration(color: Colors.blueGrey, borderRadius: BorderRadius.all(Radius.circular(0.0))),
                  ),
                  errorWidget: (context, str, error) => Material(
                    child: Image.asset('assets/images/img_not_available.jpeg', width: 197, height: 197, fit: BoxFit.cover),
                      borderRadius: BorderRadius.all(Radius.circular(0.0)),
                      clipBehavior: Clip.hardEdge),
                  imageUrl: message.split('-BREAK-')[1], width: 197, height: 197, fit: BoxFit.cover,),
                Container(color: Colors.black.withOpacity(0.4), height: 197, width: 197),
                Center(child: Icon(Icons.play_circle_fill_outlined, color: Colors.white70, size: 65)),
              ],
            ),
          ),
        ],
  ...
  Widget getContactMessage(BuildContext context, Map<String, dynamic> doc, String message, {bool saved = false}) {
    final bool isMe = doc[Dbkeys.from] == currentUserNo;
    return SizedBox(
      width: 250,
      height: 130,
          ...
          FlatButton(
              onPressed: () async {
                ...
                Query<Map<String, dynamic>> query = issearchraw == true
                    ? FirebaseFirestore.instance.collection(DbPaths.collectionusers).where(Dbkeys.phoneRaw, isEqualTo: formattedphone ?? peerphone).limit(1)
                    : FirebaseFirestore.instance.collection(DbPaths.collectionusers).where(Dbkeys.phone, isEqualTo: formattedphone ?? peerphone).limit(1);
                ...
                await query.get().then((user) {
                  ...
                  if (isUser) {
                    ...
                    Query<Map<String, dynamic>> queryretrywithoutzero = issearchraw == true
                      ? FirebaseFirestore.instance.collection(DbPaths.collectionusers).where(Dbkeys.phoneRaw, isEqualTo: formattedphone == null
                        ? peerphone!.substring(1, peerphone!.length)
                        : formattedphone!.substring(1, formattedphone!.length)).limit(1)
                      : FirebaseFirestore.instance.collection(DbPaths.collectionusers).where(Dbkeys.phoneRaw, isEqualTo: formattedphone == null
                        ? peerphone!.substring(1, peerphone!.length)
                        : formattedphone!.substring(1, formattedphone!.length)).limit(1);
                    queryretrywithoutzero.get().then((user) {
                      ...
                      if (isUser) {
                        Map<String, dynamic> peer = user.docs[0].data();
                        widget.model.addUser(user.docs[0]);
                        Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) => new ChatScreen(
    ...
    return SeenProvider(
        ...
        child: Bubble(
            ...
            isBroadcastMssg: doc.containsKey(Dbkeys.isbroadcast) == true
                ...
                : false,
            messagetype: doc[Dbkeys.messageType] == MessageType.text.index ? MessageType.text
                : doc[Dbkeys.messageType] == MessageType.contact.index ? MessageType.contact
                : doc[Dbkeys.messageType] == MessageType.location.index ? MessageType.location
                : doc[Dbkeys.messageType] == MessageType.image.index ? MessageType.image
                : doc[Dbkeys.messageType] == MessageType.video.index ? MessageType.video
                : doc[Dbkeys.messageType] == MessageType.doc.index ? MessageType.doc
                : doc[Dbkeys.messageType] == MessageType.audio.index ? MessageType.audio
                : MessageType.text,
            child: doc[Dbkeys.messageType] == MessageType.text.index
                ? getTextMessage(isMe, doc, saved)
                : doc[Dbkeys.messageType] == MessageType.location.index ? getLocationMessage(doc, doc[Dbkeys.content], saved: false)
                : doc[Dbkeys.messageType] == MessageType.doc.index ? getDocmessage(context, doc, doc[Dbkeys.content], saved: false)
                : doc[Dbkeys.messageType] == MessageType.audio.index ? getAudiomessage(context, doc, doc[Dbkeys.content], isMe: isMe, saved: false)
                : doc[Dbkeys.messageType] == MessageType.video.index ? getVideoMessage(context, doc, doc[Dbkeys.content], saved: false)
                : doc[Dbkeys.messageType] == MessageType.contact.index ? getContactMessage(context, doc, doc[Dbkeys.content], saved: false)
  ...
  replyAttachedWidget(BuildContext context, var doc) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(color: fiberchatWhite.withOpacity(0.55), borderRadius: BorderRadius.all(Radius.circular(10))),
          child: Stack(
            children: [
              Container(
                  margin: EdgeInsetsDirectional.all(4),
                  decoration: BoxDecoration(color: fiberchatGrey.withOpacity(0.1), borderRadius: BorderRadius.all(Radius.circular(8))),
                  child: Row(children: [
                    Container(
                      decoration: BoxDecoration(
                        color: doc[Dbkeys.from] == currentUserNo ? fiberchatgreen : Colors.purple,
                      ...
                      child: Column(
                        ...
                        children: [
                          Padding(
                            ...
                            child: Text(
                              doc[Dbkeys.from] == currentUserNo ? getTranslated(this.context, 'you') : Fiberchat.getNickname(peer!)!,
                              overflow: TextOverflow.ellipsis,
                              style: TextStyle(fontWeight: FontWeight.bold, color: doc[Dbkeys.from] == currentUserNo ? fiberchatgreen : Colors.purple),
                            ),
                          ),
                          SizedBox(height: 8),
                          doc[Dbkeys.messageType] == MessageType.text.index
                              ...
                              : doc[Dbkeys.messageType] == MessageType.doc.index
                                  ...
                                  : Text(
                                      getTranslated(
                                          this.context,
                                          doc[Dbkeys.messageType] == MessageType.image.index ? 'nim'
                                              : doc[Dbkeys.messageType] == MessageType.video.index ? 'nvm'
                                              : doc[Dbkeys.messageType] == MessageType.audio.index ? 'nam'
                                              : doc[Dbkeys.messageType] == MessageType.contact.index ? 'ncm'
                                              : doc[Dbkeys.messageType] == MessageType.location.index ? 'nlm'
                                              : doc[Dbkeys.messageType] == MessageType.doc.index ? 'ndm' : ''),
                                      overflow: TextOverflow.ellipsis,
                                    ),
                        ],
                      ),
              ...
              doc[Dbkeys.messageType] == MessageType.text.index || doc[Dbkeys.messageType] == MessageType.location.index
                            ...
                            child: ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                ...
                                errorWidget: (context, str, error) => Material(
                                  ...
                                  borderRadius: BorderRadius.all(Radius.circular(8.0)),
                                  clipBehavior: Clip.hardEdge),
                                imageUrl: doc[Dbkeys.messageType] == MessageType.video.index ? '' : doc[Dbkeys.content],
                                width: 60,
                                height: 60,
                                fit: BoxFit.cover),
                            ),
                                        ...
                                        child: Stack(
                                          children: [
                                            CachedNetworkImage(
                                              ...
                                              height: 74,
                                              fit: BoxFit.cover),
                                            Container(color: Colors.black.withOpacity(0.4), height: 74, width: 74,),
                                            Center(child: Icon(Icons.play_circle_fill_outlined, color: Colors.white70, size: 25)),
                                          ],
                                        ),
                                      ))))
                          : Positioned(
                              ...
                              child: Container(
                                  ...
                                  child: ClipRRect(
                                      borderRadius: BorderRadius.only(
                                          ...
                                          bottomLeft: Radius.circular(0)),
                                      child: Container(
                                          color: doc[Dbkeys.messageType] == MessageType.doc.index ? Colors.yellow[800]
                                              : doc[Dbkeys.messageType] == MessageType.audio.index ? Colors.green[400]
                                              : doc[Dbkeys.messageType] == MessageType.location.index ? Colors.red[700]
                                              : doc[Dbkeys.messageType] == MessageType.contact.index ? Colors.blue[400] : Colors.cyan[700],
                                          height: 74,
                                          width: 74,
                                          child: Icon(
                                            doc[Dbkeys.messageType] == MessageType.doc.index ? Icons.insert_drive_file
                                                : doc[Dbkeys.messageType] == MessageType.audio.index ? Icons.mic_rounded
                                                : doc[Dbkeys.messageType] == MessageType.location.index ? Icons.location_on
                                                : doc[Dbkeys.messageType] == MessageType.contact.index ? Icons.contact_page_sharp
  ...
  Widget buildReplyMessageForInput(BuildContext context) {
    return Flexible(
      child: Container(
          ...
          decoration: BoxDecoration(color: fiberchatWhite, borderRadius: BorderRadius.all(Radius.circular(10))),
          child: Stack(
            children: [
              Container(
                  margin: EdgeInsetsDirectional.all(4),
                  ...
                  child: Row(children: [
                    Container(
                      decoration: BoxDecoration(
                        color: replyDoc![Dbkeys.from] == currentUserNo ? fiberchatgreen : Colors.purple,
                        borderRadius: BorderRadius.only(
                            topRight: Radius.circular(0),
                            bottomRight: Radius.circular(0),
                            topLeft: Radius.circular(10),
                      ...
                      child: Column(
                        ...
                        children: [
                          Padding(
                            padding: EdgeInsets.only(right: 30),
                            child: Text(
                              replyDoc![Dbkeys.from] == currentUserNo ? getTranslated(this.context, 'you') : Fiberchat.getNickname(peer!)!,
                              overflow: TextOverflow.ellipsis,
                              style: TextStyle(
                          ...
                          replyDoc![Dbkeys.messageType] == MessageType.text.index
                              ? Text(replyDoc![Dbkeys.content], overflow: TextOverflow.ellipsis, maxLines: 2)
                              : replyDoc![Dbkeys.messageType] == MessageType.doc.index
                                  ? Container(
                                      width: MediaQuery.of(context).size.width - 125,
                                      padding: const EdgeInsets.only(right: 55),
                                      child: Text(replyDoc![Dbkeys.content].split('-BREAK-')[1], overflow: TextOverflow.ellipsis, maxLines: 2))
                                  : Text(
                                      getTranslated(
                                          this.context,
                                          replyDoc![Dbkeys.messageType] == MessageType.image.index ? 'nim'
                                              : replyDoc![Dbkeys.messageType] == MessageType.video.index ? 'nvm'
                                              : replyDoc![Dbkeys.messageType] == MessageType.audio.index ? 'nam'
                                              : replyDoc![Dbkeys.messageType] == MessageType.contact.index ? 'ncm'
                                              : replyDoc![Dbkeys.messageType] == MessageType.location.index ? 'nlm'
                                              : replyDoc![Dbkeys.messageType] ==MessageType.doc.index ? 'ndm' : ''),
                                      overflow: TextOverflow.ellipsis,
                                      maxLines: 2,
                                    ),
                        ],
                            ...
                            child: ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                placeholder: (context, url) => Container(
                                  ...
                                  padding: EdgeInsets.all(8.0),
                                  decoration: BoxDecoration(color: Colors.blueGrey[200], borderRadius: BorderRadius.all(Radius.circular(8.0))),
                                ),
                                errorWidget: (context, str, error) => Material(
                                  child: Image.asset('assets/images/img_not_available.jpeg', width: 60.0, height: 60, fit: BoxFit.cover),
                                  borderRadius: BorderRadius.all(Radius.circular(8.0)),
                                  clipBehavior: Clip.hardEdge),
                                imageUrl: replyDoc![Dbkeys.messageType] == MessageType.video.index ? '' : replyDoc![Dbkeys.content],
                                width: 60,
                                height: 60,
                                fit: BoxFit.cover,
                              ),
                                      ...
                                      child: Container(
                                        ...
                                        child: Stack(
                                          children: [
                                            ...
                                            Container(color: Colors.black.withOpacity(0.4), height: 84, width: 84),
                                            Center(child: Icon(Icons.play_circle_fill_outlined, color: Colors.white70, size: 25)),
                                          ],
                                        ),
                                      ))))
                                  ...
                                  child: ClipRRect(
                                      ...
                                      child: Container(
                                          color: replyDoc![Dbkeys.messageType] == MessageType.doc.index
                                              ? Colors.yellow[800] : replyDoc![Dbkeys.messageType] == MessageType.audio.index
                                              ? Colors.green[400] : replyDoc![Dbkeys.messageType] == MessageType.location.index
                                              ? Colors.red[700] : replyDoc![Dbkeys.messageType] == MessageType.contact.index
                                              ? Colors.blue[400] : Colors.cyan[700],
                                          height: 84,
                                          width: 84,
                                          child: Icon(
                                            replyDoc![Dbkeys.messageType] == MessageType.doc.index
                                                ? Icons.insert_drive_file : replyDoc![Dbkeys.messageType] == MessageType.audio.index
                                                ? Icons.mic_rounded : replyDoc![Dbkeys.messageType] == MessageType.location.index
                                                ? Icons.location_on : replyDoc![Dbkeys.messageType] == MessageType.contact.index
                                                ? Icons.contact_page_sharp : Icons.insert_drive_file,
                                            color: Colors.white,
                                            size: 35))))),
              Positioned(
                right: 7,
                ...
                child: InkWell(
                  ...
                  child: Container(
                    width: 15,
                    height: 15,
                    decoration: new BoxDecoration(color: Colors.white.withOpacity(0.9), shape: BoxShape.circle),
                    child: new Icon(Icons.close, color: Colors.blueGrey, size: 13))),
              )
            ],
          )),
    );
  ...
  Widget buildTempMessage(BuildContext context, MessageType type, content, timestamp, delivered, tempDoc) {
    ...
    return SeenProvider(
        ...
        child: Bubble(
          mssgDoc: tempDoc,
          is24hrsFormat: observer.is24hrsTimeformat,
          isMssgDeleted: ((tempDoc.containsKey(Dbkeys.hasRecipientDeleted) && tempDoc.containsKey(Dbkeys.hasSenderDeleted)) == true)
              ? (isMe == true
                  ? (tempDoc[Dbkeys.from] == widget.currentUserNo ? tempDoc[Dbkeys.hasSenderDeleted] : false)
                  : (tempDoc[Dbkeys.from] != widget.currentUserNo ? tempDoc[Dbkeys.hasRecipientDeleted] : false))
              : false,
          isBroadcastMssg: false,
          messagetype: type,
          child: type == MessageType.text ? getTempTextMessage(content, tempDoc)
              : type == MessageType.location ? getLocationMessage(tempDoc, content, saved: false)
              : type == MessageType.doc ? getDocmessage(context, tempDoc, content, saved: false)
              : type == MessageType.audio ? getAudiomessage(context, tempDoc, content, saved: false, isMe: isMe)
              : type == MessageType.video ? getVideoMessage(this.context, tempDoc, content, saved: false)
              : type == MessageType.contact ? getContactMessage(context, tempDoc, content, saved: false)
  ...
  Widget buildLoading() {
    return Positioned(
      child: isLoading
          ? Container(child: Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(fiberchatBlue))),
                    ...
                    child: Column(
                      ...
                      children: [
                        RawMaterialButton(
                            ...
                            Navigator.push(context, MaterialPageRoute(builder: (context) =>
                              MultiDocumentPicker(
                                ...
                                writeMessage: (String? url, int time) async {
                                  if (url != null) {
                                    String finalUrl = url + '-BREAK-' + basename(pickedFile!.path).toString();
                                    onSendMessage(this.context, finalUrl, MessageType.doc, time);
                                  }
                                },
                              )));
                          },
                          ...
                          child: Icon(Icons.file_copy, size: 25.0, color: Colors.white),
                    ...
                    child: Column(
                      ...
                      children: [
                        RawMaterialButton(
                          ...
                          onPressed: () {
                            ...
                            Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                HybridVideoPicker(
                                  ...
                                  callback: getFileData,))).then((url) async {
                              if (url != null) {
                                Fiberchat.toast(getTranslated(this.context, 'plswait'));
                          ...
                          child: Icon(Icons.video_collection_sharp, size: 25.0, color: Colors.white),
                          padding: EdgeInsets.all(15.0),
                          shape: CircleBorder()),
                        SizedBox(height: 8),
                        Text(
                    ...
                    child: Column(
                      ...
                      children: [
                        RawMaterialButton(
                          ...
                          child: Icon(Icons.image_rounded, size: 25.0, color: Colors.white),
                          padding: EdgeInsets.all(15.0),
                          shape: CircleBorder()),
                        SizedBox(height: 8),
                        Text(
                      ...
                      children: [
                        RawMaterialButton(
                          ...
                          onPressed: () {
                            ...
                            Navigator.of(context).pop();
                            Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                AudioRecord(title: getTranslated(this.context, 'record'), callback: getFileData))).then((url) {
                              if (url != null) {
                          ...
                          child: Icon(Icons.mic_rounded, size: 25.0, color: Colors.white),
                          padding: EdgeInsets.all(15.0),
                          shape: CircleBorder(),
                        ),
                        SizedBox(height: 8),
                      ...
                      children: [
                        RawMaterialButton(
                          ...
                          onPressed: () async {
                            hidekeyboard(context);
                            Navigator.of(context).pop();
                            await _determinePosition().then((location) async {
                          ...
                          child: Icon(Icons.location_on, size: 25.0, color: Colors.white),
                          padding: EdgeInsets.all(15.0),
                          shape: CircleBorder(),
                        ),
                        SizedBox(height: 8),
  ...
  Widget buildInputAndroid(BuildContext context, bool isemojiShowing, Function refreshThisInput, bool keyboardVisible) {
    ...
    return Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          isReplyKeyboard == true ? buildReplyMessageForInput(context) : SizedBox(),
          Container(
              ...
              children: <Widget>[
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: IconButton(
                            ...
                            icon: Icon(Icons.emoji_emotions, color: fiberchatGrey),
                          ),
                        ),
                        Flexible(
                          child: TextField(
                              ...
                              children: [
                                textEditingController.text.isNotEmpty
                                    ...
                                    : SizedBox(
                                        width: 30,
                                        child: IconButton(
                                          icon: new Icon(Icons.attachment_outlined, color: fiberchatGrey),
                                          padding: EdgeInsets.all(0.0),
                                          onPressed: observer.ismediamessagingallowed == false
                                              ? () {
                                                  print(' =====> ON CLICK ATTACH <=====');
                                ...
                                textEditingController.text.length != 0
                                    ...
                                    : Container(
                                        ...
                                        child: IconButton(
                                            color: fiberchatWhite,
                                            padding: EdgeInsets.all(0.0),
                                            icon: Icon(Icons.gif_rounded, size: 40, color: fiberchatGrey),
                                            onPressed: observer.ismediamessagingallowed == false
                                                ? () {
                                                    print(' =====> ON CLICK GIF <=====');
                                                    Fiberchat.showRationale(getTranslated(this.context, 'mediamssgnotallowed'));
                ...
                Container(
                  ...
                  child: Padding(
                    ...
                    child: IconButton(
                      icon: new Icon(
                        textEditingController.text.length == 0 || isMessageLoading == true ? Icons.mic : Icons.send,
                        color: fiberchatWhite.withOpacity(0.99)),
                      onPressed: observer.ismediamessagingallowed == true ? textEditingController.text.length == 0 || isMessageLoading == true
                              ? () {
                                print(' =====> ON CLICK AUDIO RECORD <=====');
                                  hidekeyboard(context);

                                  Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                      AudioRecord(
                                          title: getTranslated(this.context, 'record'),
                                          callback: getFileData))).then((url) {
                                    if (url != null) {
                                      onSendMessage(context, url + '-BREAK-' + uploadTimestamp.toString(), MessageType.audio, uploadTimestamp);
                                    } else {}
                                  });
                    ...
                    child: Row(
                      children: [
                        IconButton(
                            color: fiberchatWhite,
                            padding: EdgeInsets.all(0.0),
                            icon: Icon(Icons.gif, size: 40, color: fiberchatGrey),
                            onPressed: observer.ismediamessagingallowed == false
                                ? () {
                                    Fiberchat.showRationale(getTranslated(this.context, 'mediamssgnotallowed'));
                                  }
                ...
                onPressed: observer.ismediamessagingallowed == true
                    ? textEditingController.text.length == 0 || isMessageLoading == true
                        ...
                        : observer.istextmessagingallowed == false
                            ...
                            : chatStatus == ChatStatus.blocked.index
                                ? null : () => onSendMessage(context, textEditingController.text, MessageType.text, DateTime.now().millisecondsSinceEpoch)
                    : () {
                        Fiberchat.showRationale(getTranslated(this.context, 'mediamssgnotallowed'));
                      },
                color: fiberchatWhite,
    ...
    await FirebaseFirestore.instance.collection(DbPaths.collectionmessages).doc(chatId).collection(chatId!).orderBy(Dbkeys.timestamp).get().then((docs) {
      if (docs.docs.isNotEmpty) {
        ...
        docs.docs.forEach((doc) {
          ...
          messages.add(Message(buildMessage(this.context, _doc),
              ...
              onTap: (_doc[Dbkeys.from] == widget.currentUserNo && _doc[Dbkeys.hasSenderDeleted] == true) == true
                  ? () {}
      ...
      if (widget.isSharingIntentForwarded == true) {
        if (widget.sharedText != null) {
          onSendMessage(this.context, widget.sharedText!, MessageType.text, DateTime.now().millisecondsSinceEpoch);
  ...
  uploadEach(int index) async {
    ...
    if (index > widget.sharedFiles!.length) {
      ...
      await getFileData(File(widget.sharedFiles![index].path), timestamp: messagetime, totalFiles: widget.sharedFiles!.length).then((imageUrl) async {
        if (imageUrl != null) {
      ...
      Save.getSavedMessages(peerNo).then((_msgDocs) {
        ...
        if (_msgDocs != null) {
          setStateIfMounted(() {
            ...
            print('loadSavedMessages => _savedMessageDocs : $_savedMessageDocs');
          });
        }
      });
    }
  ...
  List<Widget> sortAndGroupSavedMessages(BuildContext context, List<Map<String, dynamic>> _msgs) {
    ...
    _msgs.forEach((msg) {
      _savedMessages.add(Message(
        ...
        contextMenuForSavedMessage(context, msg);
          ...
          onDismiss: null,
          onTap: (msg[Dbkeys.from] == widget.currentUserNo && msg[Dbkeys.hasSenderDeleted] == true) == true
              ? () {}
    ...
    groupBy<Message, String>(_savedMessages, (msg) {
      ...
      _groupedSavedMessages.add(Center(
          child: Chip(
        label: Text(when, style: TextStyle(color: Colors.black54, fontSize: 14)),
      )));
...
//-- GROUP BY DATE ---
              ...
              children: [
                WidgetSpan(
                  child: Padding(
                    ...
                    child: Icon(Icons.lock, color: Color(0xff78754A), size: 14))),
                TextSpan(
                    text: getTranslated(this.context, 'chatencryption'),
                    style: TextStyle(color: Color(0xff78754A), height: 1.3, fontSize: 13, fontWeight: FontWeight.w400)),
              ],
            ),
          )),
      ),
  ...
  Widget buildMessages(BuildContext context) {
    if (chatStatus == ChatStatus.blocked.index) {
      return AlertDialog(
        backgroundColor: Colors.white,
        elevation: 10.0,
        title: Text(getTranslated(this.context, 'unblock') + ' ${peer![Dbkeys.nickname]}?', style: TextStyle(color: fiberchatBlack)),
        actions: <Widget>[
          myElevatedButton(
              color: fiberchatWhite,
              child: Text(getTranslated(this.context, 'cancel'), style: TextStyle(color: fiberchatBlack)),
              onPressed: () {
                Navigator.pop(context);
              }),
          myElevatedButton(
              color: fiberchatLightGreen,
              child: Text(getTranslated(this.context, 'unblock'), style: TextStyle(color: fiberchatWhite)),
              onPressed: () {
                ChatController.accept(currentUserNo, peerNo);
                setStateIfMounted(() {
                  chatStatus = ChatStatus.accepted.index;
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: sharedSecret == null || isMessageLoading == true
                  ? Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(fiberchatLightGreen)))
                  : Text(getTranslated(this.context, 'sayhi'),
                      textAlign: TextAlign.center,
                      style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatGrey, fontSize: 18))),
            ],
            controller: realtime,
          )
        : ListView(
  ...
  getPeerStatus(val) {
    ...
    if (val is bool && val == true) {
      ...
      String at = observer.is24hrsTimeformat == false ? DateFormat.jm().format(date) : DateFormat('HH:mm').format(date),
          when = getWhen(date);
    ...
    return PickupLayout(
      scaffold: Fiberchat.getNTPWrappedWidget(WillPopScope(
        onWillPop: isgeneratingSomethingLoader == true
          ? () async {  return Future.value(false); }
          : isemojiShowing == true
          ? () {
            setState(() {
              isemojiShowing = false;
          ...
          : () async {
              ...
              WidgetsBinding.instance!.addPostFrameCallback((timeStamp) async {
                ...
                if (lastSeen == peerNo)
                  await FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(currentUserNo).update({Dbkeys.lastSeen: true});
              });
                          ...
                          Scaffold(
                              ...
                              appBar: AppBar(
                                ...
                                title: InkWell(
                                  onTap: () {
                                    print('peer : $peer');
                                    print('peer : ${peer![Dbkeys.phone]}');
                                    print('widget.currentUserNo : ${widget.currentUserNo}');
                                    print('_cachedModel : $_cachedModel');
                                    // print('_cachedModel : $_cachedModel');
                                    print('widget.prefs : ${widget.prefs}');
                                    Navigator.push(context, PageRouteBuilder(opaque: false, pageBuilder: (context, a1, a2) =>
                                       ProfileView(peer!, widget.currentUserNo, _cachedModel, widget.prefs)));
                                  },
                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.start,
                                    children: [
                                          ...
                                          children: [
                                            ...
                                            chatId != null
                                                ? Text(
                                                    ...
                                                    style: TextStyle(
                                                      color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatGrey,
                                                      fontSize: 12,
                                                      fontWeight: FontWeight.w400))
                                                : Text('loading…', style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatGrey, fontSize: 12, fontWeight: FontWeight.w400)),
                                          ],
                                        ),
                                      ),
                                    ],
                                ...
                                actions: [
                                  //TODO: Pay via Wallet Number
                                  GestureDetector(
                                    onTap: () async {
                                      print(' ======> ON CLICK WALLET NUMBER <====== ');
                                      var userCodeData;
                                      print('peer : $peer');
                                      print('peer phone : ${peer![Dbkeys.phone]}');
                                      print('widget.currentUserNo : ${widget.currentUserNo}');
                                      print('_cachedModel : $_cachedModel');
                                      print('widget.prefs : ${widget.prefs}');
                                      // print('widget.prefs : ${widget.prefs[Dbkeys.phone]}');
                                        ...
                                        setState(() {
                                          APIServices.getMakeUserQRCode(val.getString(Dbkeys.sharedPrefUserId).toString()).then((userQrCodeSnapshotValue) {
                                            print('userQrCodeSnapshotValue : $userQrCodeSnapshotValue');
                                              userCodeData = UserQrCode.fromJson(json.decode(userQrCodeSnapshotValue));
                                              ...
                                              print('userCodeData username : ${userCodeData.userQrCodeData!.username}');
                                              // Navigator.pop(context);
                                              APIServices.getUserDetailWithMobileNumber('${peer![Dbkeys.phone]!.replaceAll('+', '').toString()}').then((userDetailValue) {
                                                print('userDetailValue : $userDetailValue');
                                                print('userDetailData id : ${userDetailValue.userQrCodeData!.id}');
                                                print('userDetailData accountNumber : ${userDetailValue.userQrCodeData!.accountNumber}');
                                                print('userDetailData username : ${userDetailValue.userQrCodeData!.username}');
                                                print('userDetailData email : ${userDetailValue.userQrCodeData!.email}');
                                                print('userDetailData mobile : ${userDetailValue.userQrCodeData!.mobile}');
                                                print('userDetailData deviceToken : ${userDetailValue.userQrCodeData!.deviceToken}');
                                                //TODO : 24/03/2022 : change push to pushReplacement
                                                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) =>
                                                  PayWithWalletNumberScreen(
                                                    userCodeData: userCodeData,
                                                    userDetailWithMobileNo: userDetailValue,
                                                  )));
                                              }).onError((e, stackTrace) {
                                                print('getUserDetailWithMobileNumber ERROR : $e');
                                              });
                                          }).onError((e, stackTrace) {
                                    ...
                                    child: SvgPicture.asset(Assets.paymentImage,height: 20),
                                  ),
                                  ...
                                  observer.isCallFeatureTotallyHide == true
                                    ? SizedBox()
                                    : IconButton(
                                        icon: Icon(Icons.phone, color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatgreen),
                                        onPressed: observer.iscallsallowed == false
                                            ? () {
                                              Fiberchat.showRationale(getTranslated(this.context, 'callnotallowed'));
                                              }
                                  ...
                                  SizedBox(
                                    width: observer.isCallFeatureTotallyHide == true ? 45 : 20,
                                    child: PopupMenuButton(
                                        padding: EdgeInsets.all(0),
                                        icon: Padding(
                                          padding: const EdgeInsets.only(right: 0),
                                          child: Icon(Icons.more_vert_outlined, color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack)),
                                        color: fiberchatWhite,
                                        ...
                                        itemBuilder: ((context) =>
                                            <PopupMenuItem<String>>[
                                              PopupMenuItem<String>(
                                                ...
                                                child: Text('${hidden ? getTranslated(this.context, 'unhidechat') : getTranslated(this.context, 'hidechat')}')),
                                              PopupMenuItem<String>(
                                              ...
                                              peer![Dbkeys.wallpaper] != null
                                                  ? PopupMenuItem<String>(value: 'remove_wallpaper', child: Text(getTranslated(this.context, 'removewall')))
                                                  : PopupMenuItem<String>(value: 'set_wallpaper', child: Text(getTranslated(this.context, 'setwall'))),
                                              PopupMenuItem<String>(child: Text(getTranslated(this.context, 'showtutor')), value: 'tutorial')
                                              // ignore: unnecessary_null_comparison
                                            ].toList())),
                                  ),
                                ],
                                  ...
                                  new Container(
                                    decoration: new BoxDecoration(
                                      ...
                                      image: new DecorationImage(
                                          image: peer![Dbkeys.wallpaper] == null ? AssetImage("assets/images/background.png") : Image.file(File(peer![Dbkeys.wallpaper])).image,
                                          fit: BoxFit.cover))),
                                  PageView(
                                    children: <Widget>[
                                      Column(
                                        children: [
                                          ...
                                          // Input content
                                          isBlocked() ? Container()
                                              : Platform.isAndroid ? buildInputAndroid(context,isemojiShowing, refreshInput, _keyboardVisible)
