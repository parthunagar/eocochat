  ...
  try {
    ...
    if (realtimeDoc[Dbkeys.messageType] == MessageType.image.index) {
      await FirebaseStorage.instance
          ...
          .child(getFileName(realtimeDoc[Dbkeys.from], realtimeDoc[Dbkeys.timestamp]))
          .delete();
      return true;
      ...
      Reference reference2 = FirebaseStorage.instance
          .ref("+00_CHAT_MEDIA/$chatId/")
          .child(getFileName(realtimeDoc[Dbkeys.from], '${realtimeDoc[Dbkeys.timestamp]}Thumbnail'));
    ...
    if (err.toString().contains(Dbkeys.firebaseStorageNoObjectFound1) ||
      ...
      print('all possible errors due to media already deleted but thats ok as we dont need to delete.');
      return true;
    } else {
      return false;
    }
