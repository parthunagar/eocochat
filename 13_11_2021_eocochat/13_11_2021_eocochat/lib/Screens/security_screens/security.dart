...
class _SecurityState extends State<Security> {
        ...
        child: Stack(children: [
          Scaffold(
              appBar: AppBar(
                ...
                title: Text(widget.title!, style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatBlack),),
              ...
              bottomSheet: Container(
                ...
                child: Padding(
                    ...
                    child: myElevatedButton(
                      ...
                      child: Text(getTranslated(this.context, 'done'), style: TextStyle(color: Colors.white)),
                      onPressed: () {
                        if (widget.setPasscode) {
                          if (_passCode == null)
                            Fiberchat.toast(getTranslated(this.context, 'setpasscode'));
                          if (
                              // ignore: todo
                            ...
                            print('widget.phoneNo : ${widget.phoneNo}');
                            FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(widget.phoneNo).update(data).then((_) {
                              Fiberchat.toast(getTranslated(this.context, 'welcometo') + ' $Appname!');
                              // widget.onSuccess(this.context);
                              widget.onSuccess();
                            }).onError((error, stackTrace) {
                              print('FirebaseFirestore.instance.collection ERROR 1 : $error');
                            });
                          }
                          widget.prefs.setString(Dbkeys.isSecuritySetupDone, widget.phoneNo!);
                        } else {
                          if (_formKey.currentState!.validate()) {
                            ...
                            widget.prefs.setInt(Dbkeys.passcodeTries, 0);
                            widget.prefs.setInt(Dbkeys.answerTries, 0);
                            FirebaseFirestore.instance.collection(DbPaths.collectionusers).doc(widget.phoneNo).update(data as Map<String, Object?>).then((_) {
                              widget.onSuccess(this.context);
                            }).onError((error, stackTrace) {
                              print('FirebaseFirestore.instance.collection ERROR 2 : $error');
                            });
                          }
                          widget.prefs.setString(Dbkeys.isSecuritySetupDone, widget.phoneNo!);
                        }
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      widget.setPasscode
                        ? ListTile(
                            trailing: Icon(Icons.check_circle, color: _passCode == null ? fiberchatGrey : fiberchatLightGreen, size: 35),
                            title: myElevatedButton(
                              color: fiberchatgreen,
                              child: Text(
                                getTranslated(this.context, 'setpass'),
                                style: TextStyle(color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatWhite : fiberchatWhite)),
          ...
          Positioned(
            child: isLoading
              ? Container(
                  child: Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(fiberchatBlue))),
                  color: DESIGN_TYPE == Themetype.whatsapp ? fiberchatBlack.withOpacity(0.8) : fiberchatWhite.withOpacity(0.8))
              : Container(),
          )
        ])));
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    ...
    _passCode = null;
    if (isValid)
      setState(() { _passCode = enteredPasscode;  });
  }

  _showLockScreen() {
    Navigator.push(context, PageRouteBuilder(opaque: true,
